        ...
        router.post(api) { (routerRequest, routerResponse, next) in
            do{
                ...
                if (user_name != nil && ip_adddress != nil && name != nil && password != nil){
                    ...
                    let rs = mk.sendAPI2(r: set_enable_incoming)
                    ...
                    for p in Engine.sharedInstance.getSession()?.profiles ?? []{
                        if let name : String = p["name"] as? String,let md = p["md"] as? Int,let mu = p["mu"] as? Int,let time = p["time"] as? Int{
                            //rq profile , profile limitation , profile profile-limitation
                            let rq_add = Request(api: "/tool/user-manager/profile/add", type: ApiType.ADD, p: ["name":name,"owner":"admin","starts-at":"logon"], q: nil, u: nil)
                            let rq_add_profile_limitation = Request(api: "/tool/user-manager/profile/limitation/add", type: ApiType.ADD, p: ["name":name,"owner":"admin","uptime-limit":"\(time * 60)","rate-limit-rx":"\(md * 1024 * 1024)","rate-limit-tx":"\(mu * 1024 * 1024)","rate-limit-min-rx":"\((md * 1024 * 1024) / 2)","rate-limit-min-tx":"\((mu * 1024 * 1024) / 2)"], q: nil, u: nil)
                    ...
                    routerResponse.send(json: ["status":200,"message":"ok"])
                    next()
